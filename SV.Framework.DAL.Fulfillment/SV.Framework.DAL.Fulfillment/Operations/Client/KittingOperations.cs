...
namespace SV.Framework.DAL.Fulfillment
{
    public class KittingOperations : BaseCreateInstance
    {
        ...
        public  KittingInfo GetKittingInfoByESN(string ESN, int ItemCompanyGUID)
        {
            ...
            List<PurchaseOrderKitting> poDetails = default;//null;
            List<KitRawSKU> skus = default;//null;
            using (DBConnect db = new DBConnect())
            {
                string[] arrSpFieldSeq;
        ...
        {
            List<PurchaseOrderKittingSummary> kittingSummary = default;//null;
            using (DBConnect db = new DBConnect())
            {
                string[] arrSpFieldSeq;
        ...
        {
            List<POPallet> pallets = default;//null;
            POPallet pOPallet= default;//;
            using (DBConnect db = new DBConnect())
            {
                string[] arrSpFieldSeq;
                DataTable dt = default;//new DataTable();
        ...
        {
            List<SKUiNFO> SKUs = default;//null;
        ...
        private  List<PurchaseOrderKittingSummary> PopulateKittingSummary(DataTable dt)
        {
            ...
            List<PurchaseOrderKittingSummary> kittingSummary = default;//null;
            PurchaseOrderKittingSummary kitSummary = default;//null;
            if (dt != null && dt.Rows.Count > 0)
            {
        ...
        private  List<PurchaseOrderKitting> PopulateFulfillmentDetail(DataTable dt)
        {
            ...
            List<PurchaseOrderKitting> poDetails = default;//null;// new List<PurchaseOrderShipmentDB>();
            PurchaseOrderKitting poDetail = default;//null;
            if (dt != null && dt.Rows.Count > 0)
            {
        ...
        {
            List<KitRawSKU> skus = default;//null;// new List<PurchaseOrderShipmentDB>();
            KitRawSKU skuInfo = default;//null;
            if (dt != null && dt.Rows.Count > 0)
            {
