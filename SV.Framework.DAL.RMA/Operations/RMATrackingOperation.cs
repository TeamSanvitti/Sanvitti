...
namespace SV.Framework.DAL.RMA
{
    public class RMATrackingOperation : BaseCreateInstance
    {
        ...
        private ShippingLabelResponse ShippingLabelUpdateDB(RMATrackning shippingLabelInfo, int UserID)
        {
            string errorMessage = string.Empty;

            ShippingLabelResponse response = new ShippingLabelResponse();
            response.ErrorCode = SV.Framework.Models.Fulfillment.ResponseErrorCode.MissingParameter.ToString();
            int returnValue = 0;
            using (DBConnect db = new DBConnect())
            {
                string[] arrSpFieldSeq;
