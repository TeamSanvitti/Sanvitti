...
namespace avii.Classes
{
	...
	public class clsGeneral
	{
		...
		public static  void SendEmail(string psEmailAddressTo, string psEmailAddressCC, string psSubject, string psMessage, int userID, int moduleID)
		{
            try
            {
                string sSMTP = System.Configuration.ConfigurationSettings.AppSettings["smtpClient"];
                string serviceEmail = System.Configuration.ConfigurationSettings.AppSettings["smtpFromEmail"];
                string serviceEmailPwd = System.Configuration.ConfigurationSettings.AppSettings["smtpMailPassword"];
        ...
        public static int SendEmailNew(string strEmailTo, string fromName, string strEmailSubject, string strEmailBody, string psEmailAddressCC, int userID, int moduleID, out Exception exObj, bool blnIsHtml = false)
        {
            if (string.IsNullOrEmpty(fromName))
                fromName = "Lan Global Team";
            exObj = null;
            if (!string.IsNullOrEmpty(strEmailTo))
            {
                try
                {
                    string strEmailFrom = ConfigurationManager.AppSettings["smtpFromEmail"].ToString();
                    string password = ConfigurationManager.AppSettings["smtpMailPassword"].ToString();
                    ...
                    msg.From = new MailAddress(strEmailFrom, fromName);
                  ...
                  //  msg.BodyEncoding = System.Text.Encoding.UTF8;// System.Text.Encoding.GetEncoding("utf-8");
                   ...
                   // client.UseDefaultCredentials = false;
                    ...
                    msg = null;
                  //  clsGeneral.InesrtIntoEmailLog(0, userID, strEmailBody, 1, string.Empty, moduleID, strEmailSubject, strEmailTo, psEmailAddressCC);

                    return 1;
                }
                catch (Exception ex)
                {
                    //clsGeneral.InesrtIntoEmailLog(0, userID, strEmailBody, 0, ex.Message, moduleID, strEmailSubject, strEmailTo, psEmailAddressCC);
