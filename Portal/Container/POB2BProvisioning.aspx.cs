...
{
    public partial class POB2BProvisioning : System.Web.UI.Page
    {
        ...
        private void BindPO()
        {
            ...
            if (dpCompany.SelectedIndex > 0)
            {
                ...
                List<FulfillmentAssignNonESN> nonESNList = null;
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (Session["containerEsnList"] != null)
            {
                ...
                returnRecord = ContainerProvisioningOperation.PurchaseOrderContainerESNAssign(companyID, fulfillmentNumber, userID, containerEsnList);
                if (returnRecord > 0)
                {
                ...
                if (!string.IsNullOrEmpty(poNum))
                {
                    ...
                    try
                    {
                                ...
                                if (fu.HasFile)
                                {
                                    ...
                                    if (extension == ".csv")
                                    {
                                        try
                                        {
                                            ...
                                            {
                                                string line;
                                                string esn, containerID, ICCID;//, fmupc, msl, lteICCID, lteIMSI, otksl, akey;
                                                int i = 0;
                                                while ((line = sr.ReadLine()) != null)
                                                {
                                                    ...
                                                    {
                                                        esn = containerID = ICCID = string.Empty;// fmupc = lteICCID = lteIMSI = otksl = akey = msl = string.Empty;
                                                        ...
                                                        try
                                                        {
                                                            ...
                                                            else
                                                            {
                                                                ...
                                                                if (hshESN.ContainsKey(ICCID) && !string.IsNullOrEmpty(ICCID))
                                                                {
                                                                    if (string.IsNullOrEmpty(duplicateICCIDs))
                                                                        duplicateICCIDs = ICCID;
                                                                    else
                                                                        duplicateICCIDs = duplicateICCIDs + ", " + ICCID;

                                                                }
                                                                else if (!hshESN.ContainsKey(ICCID) && !string.IsNullOrEmpty(ICCID))
                                                                {
                        ...
                        if (esnList != null && esnList.Count > 0 && columnsIncorrectFormat == false)
                        {
                            ...
                            try
                            {
                                List<ContainerESNInfo> containerEsnList = ContainerProvisioningOperation.GetContainerESNs(companyID, poNum, esnList);
