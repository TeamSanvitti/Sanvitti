...
namespace avii.RMA
{
    public partial class ManageRMAUpload : System.Web.UI.Page
    {
           ...
           // userid = 496;
            ...
            foreach (RepeaterItem item in rptRma.Items)
            {
                ...
                if (chkItem.Checked)
                {
                    ...
                    rmaDetail.PO_ID = Convert.ToInt32(hdPOID.Value);
                    ...
                    if (fu.PostedFile.ContentLength > 0)
                    {
                        ...
                        if (extension == ".csv")
                        {
                            ...
                            if (esnList != null && esnList.Count > 0 && columnsIncorrectFormat == false)
                            {
                                if (lblMsg.Text == "")
                                {
                                    List<RmaDetailModel> rmaList = rmaOperation.POValidateESN(esnList);
                                    if (rmaList != null && rmaList.Count > 0)
                                    {
                                        ...
                                        if (lblMsg.Text == "")
                                        {
                                            if (InValid == 1)
                                            {
                                                errorMessage = sb.ToString() + " invalid ESN(s)";
                                                lblMsg.Text = sb.ToString() + "  Invalid ESN(s)";
                                            }
                                        }
                                        else
                                        {
        ...
        private void BindLineItemsStatus()
        {
            if (Session["RMAdetailStatusList"] != null)
            {
                List<CustomerRMAStatus> customerRMAStatusList = Session["RMAdetailStatusList"] as List<CustomerRMAStatus>;
                ddlLineStatus.DataSource = customerRMAStatusList;
                ddlLineStatus.DataValueField = "StatusID";
                ddlLineStatus.DataTextField = "StatusDescription";
                ddlLineStatus.DataBind();
                ddlLineStatus.Items.Insert(0, new System.Web.UI.WebControls.ListItem("", "0"));
            }
        }
        ...
        protected void ddlWarrenty2_SelectedIndexChanged(object sender, EventArgs e)
        {
            foreach(RepeaterItem item in rptRma.Items)
            {
                DropDownList ddlWarrnty = item.FindControl("dpWarranty") as DropDownList;
                ddlWarrnty.SelectedValue = ddlWarrenty2.SelectedValue;
            }
        }
        ...
        protected void ddlReason2_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                DropDownList ddReason = item.FindControl("ddReason") as DropDownList;
                ddReason.SelectedValue = ddlReason2.SelectedValue;
            }
        }
        ...
        protected void ddlDisposition_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                DropDownList dpDisposition = item.FindControl("dpDisposition") as DropDownList;
                dpDisposition.SelectedValue = ddlDisposition.SelectedValue;
            }
        }

        protected void ddlLineStatus_SelectedIndexChanged(object sender, EventArgs e)
        {
            foreach (RepeaterItem item in rptRma.Items)
            {
                DropDownList ddlItemStatus = item.FindControl("ddlStatus") as DropDownList;
                ddlItemStatus.SelectedValue = ddlLineStatus.SelectedValue;
            }
        }
    }
}