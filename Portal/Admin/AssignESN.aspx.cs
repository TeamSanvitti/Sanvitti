...
{
    public partial class AssignESN : System.Web.UI.Page
    {
            ...
            if (dpCompany.SelectedIndex > 0)
            {
                if (!string.IsNullOrEmpty(poNum))
                {
                    ...
                    if (esnlist.Count > 0)
                    {
                        ...
                        List<FulfillmentAssignESN> esnList = avii.Classes.ESNAssignOperation.ValidateAssignESNScan(esnAssignment);
            ...
            if (Session["esns"] != null || Session["nonesns"] != null)
            {
                ...
                try
                {
                    using (SqlConnection con = new SqlConnection(ConnectionString))
                    {
                        ...
                        try
                        {
                            returnValue = ESNAssignOperation.AssignNonEsnItemToFulfillmentOrder(nonEsnList, fulfillmentNumber, userID, esnAssignment.CustomerAccountNumber, con, sqlTrans, cmd);
                            if (nonEsnList != null && nonEsnList.Count > 0)
                                esnXML = clsGeneral.SerializeObject(nonEsnList);
                            if (esnList != null && esnList.Count > 0)
                            {
                                ...
                                for (int i = 0; i < n; i++)
                                {
                                    ...
                                    esnXML = esnXML + clsGeneral.SerializeObject(esnToUpload);
                        ...
                        catch (Exception e2)
                        {
                            // sqlTrans.Rollback();
                            logModel.RequestData = esnXML;
                            logModel.ResponseData = e2.Message;// BaseAerovoice.SerializeObject<PurchaseOrderResponse>(response);
                            logModel.Comment = e2.Message;
                            IsException = true;
                        ...
                        if (esnList != null && esnList.Count > 0 && columnsIncorrectFormat == false && IsValid)
                        {
                            ...
                            try
                            {
                                ...
                                for (int i = 0; i < listLength; i = i + 1000)
                                {
                                    ...
                                    List<FulfillmentAssignESN> esnList2 = avii.Classes.ESNAssignOperation.ValidateAssignESNNew(esnAssignment, out poRecordCount, out invalidLTEESNMessage, out invalidESNMessage, out invalidSkuESNMessage, out esnExistsMessage, out badESNMessage, out returnValue, out poStatus, out statusID, out Maxqty);
                    ...
                    if (IsFulfillmentNumberExists == 1 || IsFulfillmentNumberExists == 10 || IsFulfillmentNumberExists == 11)
                    {
                        ...
                        if (fulfillmentNonEsnItems != null && fulfillmentNonEsnItems.Count > 0)
                        {
                            ...
                            for (int i = 0; i < fulfillmentNonEsnItems.Count; i++)
                            {
                                ...
                                //if (sku == fulfillmentNonEsnItems[i].SKU)
                                //{
                                //    index = nonESNList.Count - 1;
                                //    qty = qty + Convert.ToInt32(fulfillmentNonEsnItems[i].Qty);
                                //    CurrentStock = CurrentStock + fulfillmentNonEsnItems[i].CurrentStock;
                                //    esnitems.SKU = fulfillmentNonEsnItems[i].SKU;
                                //    esnitems.CategoryName = fulfillmentNonEsnItems[i].CategoryName;
                                //    esnitems.ProductName = fulfillmentNonEsnItems[i].ProductName;


                                //    esnitems.Qty = qty;
                                //    esnitems.CurrentStock = CurrentStock;

                                //    nonESNList[index] = esnitems;
                                //}
                                //else
                                //{
                                //    esnitems = new FulfillmentAssignNonESN();
                                //    sku = fulfillmentNonEsnItems[i].SKU;
                                //    qty = Convert.ToInt32(fulfillmentNonEsnItems[i].Qty);
                                //    CurrentStock = fulfillmentNonEsnItems[i].CurrentStock;
                                //    esnitems.SKU = fulfillmentNonEsnItems[i].SKU;
                                //    esnitems.CategoryName = fulfillmentNonEsnItems[i].CategoryName;
                                //    esnitems.ProductName = fulfillmentNonEsnItems[i].ProductName;

                                //    esnitems.Qty = qty;
                                //    esnitems.IsAssign = fulfillmentNonEsnItems[i].IsAssign;
                                //    esnitems.CurrentStock = fulfillmentNonEsnItems[i].CurrentStock;
                                //    nonESNList.Add(esnitems);
                                //}
